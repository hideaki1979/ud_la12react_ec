...
class StripeController extends Controller
{
    ...
    public function success(Request $request, OrderFulfillmentService $fulfillmentService)
    {
        // StripeセッションIDの取得
        ...
        // Stripe公式推奨: Successページでもfulfillment処理を実行
        // Webhookが遅延しても、ここで即座に処理される
        // 冪等性が保証されているため、Webhookと重複しても問題なし
        if ($order->stripe_status === 'pending') {
            $result = $fulfillmentService->fulfill($order, $sessionId);

            if (! $result['success'] && ! $result['already_processed']) {
                Log::error('Success Page: Fulfillment処理に失敗しました。Order ID: '.$order->id);
        ...
        } elseif ($order->stripe_status === 'failed') {
            return Inertia::render('Checkout/OrderComplete', [
                'error' => '注文処理中に問題が発生しました。後ほど注文履歴をご確認ください。',
            ]);
        } else {
